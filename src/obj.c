...
coil_err_t coil_obj_init(coil_object_t *obj, int flags) {
  ...
  coilt_memset(obj, 0, sizeof(coil_object_t));
  ...
  if (flags & COIL_OBJ_INIT_EMPTY) {
    // Set magic number
    coilt_memcpy(obj->header.magic, COIL_MAGIC, sizeof(obj->header.magic));
...
static coil_err_t coil_obj_init_sections_array(coil_object_t *obj) {
  ...
  obj->sections = (coil_section_t*)coilt_calloc(obj->header.section_count, sizeof(coil_section_t));
  if (obj->sections == NULL) {
...
coil_err_t coil_obj_load_section(coil_object_t *obj, coil_u16_t index, coil_section_t *sect, int mode) {
  ...
  coilt_memset(sect, 0, sizeof(coil_section_t));
  ...
  if (header->size > 0 && header->type != COIL_SECTION_NOBITS) {
    if (obj->is_mapped) {
      ...
      coilt_memcpy(sect->data, obj->memory + header->offset, header->size);
    } else if (obj->fd >= 0) {
  ...
  if (obj->sections != NULL && index < obj->header.section_count) {
    ...
    coilt_memcpy(&obj->sections[index], sect, sizeof(coil_section_t));
  }
...
coil_err_t coil_obj_create_section(coil_object_t *obj, coil_u8_t type, const char *name, 
  ...
  coil_section_header_t *new_headers = (coil_section_header_t*)coilt_realloc(
    obj->sectheaders, 
  ...
  *index = obj->header.section_count;
  ...
  if (sect != NULL) {
    ...
    coil_section_t *new_sections = (coil_section_t*)coilt_realloc(
      obj->sections, 
