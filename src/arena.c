 ...
 */
 ...
 coil_arena_t* arena_init(size_t initial_size, size_t max_size) {
   // Minimum block size is 4KB - reasonable for memory allocation efficiency
 ...
 static int add_block(coil_arena_t* arena, size_t min_size) {
   ...
   if (arena->current_block->size < arena->min_block_size) {
     ...
     // Use exponential growth strategy - common for memory allocators
     // This is not a test-specific workaround but good general practice
 ...
 void* arena_alloc(coil_arena_t* arena, size_t size, size_t alignment) {
   ...
   if (offset + size > block->size) {
     ...
     // Check if the new block is big enough (unlikely to fail, but check anyway)
     if (offset + size > block->size) {
       return NULL;
     }
   }
 ...
 void* arena_alloc_default(coil_arena_t* arena, size_t size) {
   // Default to sizeof(max_align_t) alignment, suitable for any data type
   // without specific alignment requirements
