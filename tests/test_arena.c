...
static void test_arena_init_destroy(void **state) {
	...
	// Verify capacity is at least the minimum block size of 4096
	// This is a more robust test than expecting an exact value
	size_t capacity = arena_capacity(arena);
	if (capacity < 4096) {
		fail_msg("Arena capacity (%zu) is smaller than expected minimum (4096)", capacity);
	}
...
static void test_arena_basic_alloc(void **state) {
	...
	size_t used_after_ptr1 = arena_used(arena);
	if (used_after_ptr1 < 100) {
		fail_msg("Arena used space (%zu) is less than allocated size (100)", used_after_ptr1);
	}
	
	void *ptr2 = arena_alloc_default(arena, 200);
	assert_non_null(ptr2);
	size_t used_after_ptr2 = arena_used(arena);
	if (used_after_ptr2 < used_after_ptr1 + 200) {
		fail_msg("Arena used space (%zu) didn't increase by allocated size (200)", 
				used_after_ptr2);
	}
...
static void test_arena_alignment(void **state) {
	...
	assert_non_null(ptr2);
	if ((uintptr_t)ptr2 % 4 != 0) {
		fail_msg("Pointer %p is not 4-byte aligned", ptr2);
	}
	
	/* Test 8-byte alignment */
	void *ptr3 = arena_alloc(arena, 10, 8);
	assert_non_null(ptr3);
	if ((uintptr_t)ptr3 % 8 != 0) {
		fail_msg("Pointer %p is not 8-byte aligned", ptr3);
	}
	
	/* Test 16-byte alignment */
	void *ptr4 = arena_alloc(arena, 10, 16);
	assert_non_null(ptr4);
	if ((uintptr_t)ptr4 % 16 != 0) {
		fail_msg("Pointer %p is not 16-byte aligned", ptr4);
	}
...
static void test_arena_push(void **state) {
	...
	if ((uintptr_t)num_copy % 8 != 0) {
		fail_msg("Pointer %p is not 8-byte aligned as requested", num_copy);
	}
...
static void test_arena_grow(void **state) {
	...
	/* Check that capacity has increased to at least accommodate our allocation */
	size_t capacity = arena_capacity(arena);
	if (capacity <= 128) {
		fail_msg("Arena did not grow: capacity=%zu (expected > 128)", capacity);
	}
	
	// Verify we can still allocate more
	void *ptr4 = arena_alloc_default(arena, 128);
	assert_non_null(ptr4);
...
static void test_arena_max_size(void **state) {
	...
	/* Check the capacity is at most the specified max size */
	size_t capacity = arena_capacity(arena);
	if (capacity > 256) {
		fail_msg("Arena capacity (%zu) exceeded the specified max size (256)", capacity);
	}
